 ...
 * 
 * Bu composable studentning darslarini olish va 
 * Vue Cal uchun formatlash uchun javobgar
 * 
 * Clean Architecture tamoyillari:
 * - Single Responsibility: Faqat student schedule logikasi
 * - Separation of Concerns: Business logic composable'da
 * - Reusability: Qayta ishlatiladigan logika
 */
export function useStudentSchedule() {
  ...
  const transformLessonToEvent = (lesson) => {
    ...
    // Teacher ma'lumotlarini qo'shish
    const teacherName = lesson.teacherId?.name && lesson.teacherId?.surname
  ...
  /**
   * Student lessons'ni yuklash
   */
  const fetchStudentLessons = async () => {
    // Faqat student roli uchun ishlash
    if (authStore.role !== 'student') {
      error.value = 'Student ma\'lumotlari topilmadi'
      return
    }
    ...
    try {
      // Backend avtomatik student ID va classId bo'yicha filter qiladi
